...
begin
  ...
  try
    if ((Self.config.IN_Start > -1) and (not AC.running) and (AC.clientConnected) and
      (RCSi.GetInput(Self.config.RCSAdr, Self.config.IN_Start) = isOn)) then
      AC.Start();

    if ((Self.config.IN_Pause > -1) and (AC.running) and (RCSi.GetInput(Self.config.RCSAdr, Self.config.IN_Pause)
      = isOn)) then
      AC.Pause();

    if ((Self.config.IN_Stop > -1) and (AC.running) and (RCSi.GetInput(Self.config.RCSAdr, Self.config.IN_Stop) = isOn))
    then
      AC.Stop();
    ...
    begin
      ...
      begin
        ...
        if (Self.RepeatDown) then
          Self.RepeatDown := false;
      end;
