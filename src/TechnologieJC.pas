      ...
      if (Self.fproperties.Trat > -1) then
       begin
        ...
        if (TBlkTrat(Blk).Zadost) then
         begin
          ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'ProbÌh· û·dost '+Blk.GetGlobalSettings().name, (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
       ...
       if (Integer((Usek as TBlkUsek).Zaver) > 0) then
        begin
         ...
         if (((Nav as TBlkSCom).Navest > 0) and ((Nav as TBlkSCom).DNjc = Self)) then
          begin
           ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'Chyba povolovacÌ n·vÏsti '+Nav.GetGlobalSettings().name, (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
           Self.RusJCWithoutBlk();
          end;
