        ...
        if (neprofil <> nil) then
         begin
          if (neprofil.Obsazeno <> TUsekStav.uvolneno) then
           begin
            if (Self.fstaveni.SenderPnl <> nil) and (Self.fstaveni.SenderOR <> nil) then
              ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'NeuvolnÏn ' + neprofil.GetGlobalSettings.name,
                  (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
      ...
      if (Self.fproperties.Trat > -1) then
       begin
        ...
        if (TBlkTrat(Blk).Zadost) then
         begin
          if (Self.fstaveni.SenderPnl <> nil) and (Self.fstaveni.SenderOR <> nil) then
            ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'ProbÌh· û·dost '+Blk.GetGlobalSettings().name,
              (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
       ...
       if (Integer((Usek as TBlkUsek).Zaver) > 0) then
        begin
         ...
         if (((Nav as TBlkSCom).Navest > 0) and ((Nav as TBlkSCom).DNjc = Self)) then
          begin
           if (Self.fstaveni.SenderPnl <> nil) and (Self.fstaveni.SenderOR <> nil) then
             ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'Chyba povolovacÌ n·vÏsti '+Nav.GetGlobalSettings().name,
                (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
           Self.RusJCWithoutBlk();
          end;
     ...
     begin
      ...
      if (Self.fproperties.TypCesty = TJCType.vlak) then
       begin
        ...
        Usek := (Nav as TBlkSCom).UsekPred;
        if ((Self.RozpadRuseniBlok = 1) and ((Usek as TBlkUsek).IsSouprava()) and ((Usek as TBlkUsek).Souprava = (Blk as TBlkUsek).Souprava)) then
         begin
          writelog('JC '+Self.nazev+': smazana souprava '+Soupravy.GetSprNameByIndex((Usek as TBlkUsek).Souprava)+' z bloku '+Usek.GetGlobalSettings().name, WR_SPRPREDAT, 0);
          (Usek as TBlkUsek).RemoveSoupravy();
         end;
...
var UsekActual,UsekDalsi,Nav:TBlk;
 begin
  ...
  (UsekDalsi as TBlkUsek).AddSoupravaL((UsekActual as TBlkUsek).Souprava);
      ...
      for i := 0 to Self.fproperties.Prejezdy.Count-1 do
       begin
        ...
        if ((Blk as TBlkPrejezd).Stav.basicStav <> TBlkPrjBasicStav.uzavreno) then
          if (Self.fstaveni.SenderPnl <> nil) and (Self.fstaveni.SenderOR <> nil) then
            ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'Neuzav¯en '+(Blk as TBlkPrejezd).GetGlobalSettings().name,
              (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
       end;//for i
    end;//case 13
...
begin
 ...
 if (Self.fstaveni.SenderPnl <> nil) and (Self.fstaveni.SenderOR <> nil) then
   ORTCPServer.BottomError(Self.fstaveni.SenderPnl, 'Nep¯estavena '+(Sender as TBlkVyhybka).GetGlobalSettings.name,
     (Self.fstaveni.SenderOR as TOR).ShortName, 'TECHNOLOGIE');
 Self.CancelStaveni('', true);
 Self.RusJC();
end;//procedure
