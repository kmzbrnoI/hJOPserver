     ...
     101:begin
      ...
      if (Self.fstaveni.ncBariery.Count <> Self.fstaveni.ncBarieryCntLast) then
       begin
        ...
        ORTCPServer.Potvr(Self.fstaveni.SenderPnl, Self.NC_PS_Callback, Self.fstaveni.SenderOR as TOR, 'ZapnutÌ p¯ivol·vacÌ n·vÏsti', TBlky.GetBlksList(Navestidlo, Blk), Self.BarieryNCToPotvr(Self.fstaveni.ncBariery));
       end;
 ...
 for i := 0 to bariery.Count-1 do
  begin
   case (bariery[i].typ) of
    ...
    _JCB_PREJEZD_NOUZOVE_OTEVREN : Result.Add(GetPSPodminka(bariery[i].blok, 'NouzovÏ otev¯en'));
    ...
    _JCB_VYHYBKA_KONC_POLOHA     : Result.Add(GetPSPodminka(bariery[i].blok, 'NenÌ spr·vn· poloha'));
    _JCB_VYHYBKA_NOUZ_ZAVER      : Result.Add(GetPSPodminka(bariery[i].blok, 'NenÌ zaveden nouzov˝ z·vÏr'));
    _JCB_VYHYBKA_NESPAVNA_POLOHA : Result.Add(GetPSPodminka(bariery[i].blok, 'NenÌ spr·vn· poloha'));

    _JCB_TRAT_ZAK                : Result.Add(GetPSPodminka(bariery[i].blok, 'Z·kaz odjezdu'));
    _JCB_TRAT_ZAVER              : Result.Add(GetPSPodminka(bariery[i].blok, 'Z·vÏr'));
    _JCB_TRAT_OBSAZENO           : Result.Add(GetPSPodminka(bariery[i].blok, 'Obsazeno'));
    _JCB_TRAT_ZADOST             : Result.Add(GetPSPodminka(bariery[i].blok, 'ProbÌh· û·dost'));
    _JCB_TRAT_NESOUHLAS          : Result.Add(GetPSPodminka(bariery[i].blok, 'Nesouhlas'));
    _JCB_TRAT_NO_BP              : Result.Add(GetPSPodminka(bariery[i].blok, 'Blokov· podmÌnka nezavedena'));

    _JCB_ZAMEK_NEUZAMCEN         : Result.Add(GetPSPodminka(bariery[i].blok, 'NeuzamËen'));
    _JCB_ZAMEK_NOUZ_ZAVER        : Result.Add(GetPSPodminka(bariery[i].blok, 'NenÌ zaveden nouzov˝ z·vÏr'));
...
begin
 ...
 for i := 0 to Self.fproperties.Vyhybky.Count-1 do
  begin
   Blky.GetBlkByID(Self.fproperties.Vyhybky[i].Blok, blk);
   if ((blk as TBlkVyhybka).vyhZaver) then Result.Add(GetPSPodminka(blk, 'RuöenÌ NZ'));
   if ((blk as TBlkVyhybka).GetSettings.spojka > -1) then
    begin
     Blky.GetBlkByID((blk as TBlkVyhybka).GetSettings.spojka, blk2);
     if ((blk2 as TBlkVyhybka).vyhZaver) then Result.Add(GetPSPodminka(blk2, 'RuöenÌ NZ'));
    end;
  end;//for i
 ...
 for i := 0 to Self.fproperties.podminky.zamky.Count-1 do
  begin
   Blky.GetBlkByID(Self.fproperties.podminky.zamky[i].Blok, blk);
   if ((blk as TBlkZamek).nouzZaver) then Result.Add(GetPSPodminka(blk, 'RuöenÌ NZ'));
  end;//for i
end;//function
