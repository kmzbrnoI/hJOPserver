      ...
      if (Self.fproperties.Trat > -1) then
       begin
        ...
        if ((((Blk as TBlkTrat).Smer <> Self.fproperties.TratSmer) and
          ...
          or (not TBlkTU(blk2).sectReady) or ((Blk as TBlkTrat).ZAK)) then
         begin
     ...
     101:begin
      ...
      if (Self.fstaveni.ncBariery.Count <> Self.fstaveni.ncBarieryCntLast) then
       begin
        ...
        ORTCPServer.Potvr(Self.fstaveni.SenderPnl, Self.NC_PS_Callback, Self.fstaveni.SenderOR as TOR, 'Zapnutí pøivolávací návìsti', TBlky.GetBlksList(Navestidlo, Blk), Self.BarieryNCToPotvr(Self.fstaveni.ncBariery));
       end;
...
begin
 ...
 if (Self.fproperties.Trat > -1) then
  begin
   Blky.GetBlkByID(Self.fproperties.Trat, Blk);
   Blky.GetBlkByID(Self.fproperties.Useky[Self.fproperties.Useky.Count-1], Blk2);
   if (not (TBlkTU(Blk2).sectReady) or ((Blk as TBlkTrat).ZAK) or
...
begin
 ...
 case (Bariera.typ) of
  ...
  _JCB_TRAT_ZAK                : Result[1] := GetUPOLine('Zákaz odjezdu');
