...
procedure TReset.ResetProgramu;
 begin
   writelog('Probiha resetov·nÌ programu...', WR_MESSAGE);

   //nastaveni kurzoru na normalni a jeho zobrazeni
   Screen.Cursor := crDefault;
   ShowCursor(true);
   writelog('Kurzor zobrazen a nastaven na norm·lnÌ', WR_MESSAGE);

   //detekce postavenych VC a ruseni
   JCDB.RusAllJC();

   writelog('Vyresetov·n program', WR_MESSAGE);
 end;//procedure
...
procedure TReset.VynulovatVystupy;
 begin
  MTB.NullOutputs;
  writelog('Vynulov·ny existujÌcÌ v˝stupy', WR_MESSAGE);
 end;//procedure
