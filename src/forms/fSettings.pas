...
begin
  ...
  if (CHB_SS_In_Start.Checked) then
    ...
    SE_SS_In_Start.Value := -1;
  if (CHB_SS_In_Pause.Checked) then
    SE_SS_In_Pause.Value := data.IN_Pause
  else
    SE_SS_In_Pause.Value := -1;
  if (CHB_SS_In_Stop.Checked) then
    SE_SS_In_Stop.Value := data.IN_Stop
  else
    SE_SS_In_Stop.Value := -1;
  if (CHB_SS_In_Repeat.Checked) then
    SE_SS_In_Repeat.Value := data.IN_Repeat
  else
    SE_SS_In_Repeat.Value := -1;
  if (CHB_SS_In_Reset.Checked) then
    SE_SS_In_Reset.Value := data.IN_Reset
  else
    SE_SS_In_Reset.Value := -1;

  if (CHB_SS_Out_Ready.Checked) then
    SE_SS_Out_Ready.Value := data.OUT_Ready
  else
    SE_SS_Out_Ready.Value := -1;
  if (CHB_SS_Out_Start.Checked) then
    SE_SS_Out_Start.Value := data.OUT_Start
  else
    SE_SS_Out_Start.Value := -1;
  if (CHB_SS_Out_Pause.Checked) then
    SE_SS_Out_Pause.Value := data.OUT_Pause
  else
    SE_SS_Out_Pause.Value := -1;
  if (CHB_SS_Out_Stop.Checked) then
    SE_SS_Out_Stop.Value := data.OUT_Stop
  else
    SE_SS_Out_Stop.Value := -1;
  if (CHB_SS_OUT_Opakovani.Checked) then
    SE_SS_Out_Opakovani.Value := data.OUT_Repeat
  else
    SE_SS_Out_Opakovani.Value := -1;
...
begin
  ...
  if (CHB_SS_In_Start.Checked) then
    ...
    data.IN_Start := -1;
  if (CHB_SS_In_Pause.Checked) then
    data.IN_Pause := Self.SE_SS_In_Pause.Value
  else
    data.IN_Pause := -1;
  if (CHB_SS_In_Stop.Checked) then
    data.IN_Stop := Self.SE_SS_In_Stop.Value
  else
    data.IN_Stop := -1;
  if (CHB_SS_In_Repeat.Checked) then
    data.IN_Repeat := Self.SE_SS_In_Repeat.Value
  else
    data.IN_Repeat := -1;
  if (CHB_SS_In_Reset.Checked) then
    data.IN_Reset := Self.SE_SS_In_Reset.Value
  else
    data.IN_Reset := -1;

  if (CHB_SS_Out_Start.Checked) then
    data.OUT_Start := Self.SE_SS_Out_Start.Value
  else
    data.OUT_Start := -1;
  if (CHB_SS_Out_Pause.Checked) then
    data.OUT_Pause := Self.SE_SS_Out_Pause.Value
  else
    data.OUT_Pause := -1;
  if (CHB_SS_Out_Stop.Checked) then
    data.OUT_Stop := Self.SE_SS_Out_Stop.Value
  else
    data.OUT_Stop := -1;
  if (CHB_SS_Out_Ready.Checked) then
    data.OUT_Ready := Self.SE_SS_Out_Ready.Value
  else
    data.OUT_Ready := -1;
  if (CHB_SS_OUT_Opakovani.Checked) then
    data.OUT_Repeat := Self.SE_SS_Out_Opakovani.Value
  else
    data.OUT_Repeat := -1;
