...
{ Tato unita se stara o to, aby u klientu AC registrovala, o zmenach kterych
  bloku chteji byt AC informovani a posilala tyto notifikace do bloku. }
...
begin
 if ((parsed.Count >= 5) and (UpperCase(parsed[3]) = 'BLOCKS')) then
  begin
   if ((parsed.Count >= 6) and (UpperCase(parsed[4]) = 'REGISTER')) then begin
   ...
   end else if ((parsed.Count >= 6) and (UpperCase(parsed[4]) = 'UNREGISTER')) then begin
    blks := TStringList.Create();
    try
     ExtractStringsEx([','], [], parsed[5], blks);
     for blk in blks do
...
var blkid: Integer;
begin
 if (not Self.clientToBlks.ContainsKey(client)) then Exit();
