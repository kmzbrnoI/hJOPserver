...
begin
  ...
  Result := Result + ownConvert.BoolToStr10(Self.data.dir_L);
  ...
  Result := Result + ownConvert.BoolToStr10(Self.data.announcement) + ';';
...
begin
  ...
  if (msignal.areas.Count < 1) then
    Exit();
  area := msignal.areas[0];

  if ((not Assigned(area.announcement)) or (not area.announcement.available)) then
    Exit();
...
begin
  for var podj: TPOdj in Self.data.podj.Values do
    podj.Free();
...
begin
  ...
  for var blkId: Integer in Self.data.podj.Keys do
