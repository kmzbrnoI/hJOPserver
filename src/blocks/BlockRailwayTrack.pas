...
type
 ...
 TBlkRTSettings = record
   ...
   speeds: TDictionary<Cardinal, Cardinal>;                                     // rychlost v tratovem useku pro danou tridu prechodnosti; trida prechodnosti 0 je fallback v pripade neexistence zaznamu
 end;
 ...
 TBlkRTState = record
  ...
  stopSpeed: Integer;                                                           // tady si pamatuji, jakou rychlost mela souprava puvodne (mela by to byt tratova, toto je tu pro rozsireni zastavek nejen do trati)
 ...
 TBlkRT = class(TBlkTrack)
  ...
  private
   ...
   m_signalCovering, m_railway: TBlk;                                           // odkaz na kryci navestidla v lichem a sudem smeru
    ...
    function GetSignalCover(): TBlk;                                            // vrati kryci navestidlo TU, jinak nil (pokud blok neni v aktualnim smeru trati kryty zadnym navestidlem)
    function GetSignalCoverL(): TBlk;
    function GetSignalCoverS(): TBlk;
    function GetRailwayReady(): Boolean;                                        // vrati, jestli je trat zpusobila prace: jestli existuje a ma smer AtoB nebo BtoA, viz property \tratSmer
    function GetPrevRT(): TBlkRT;                                               // vrati predchozi TU v zavislosti na smeru trati, pokud smer neni AtoB nebo BtoA, vrati nil, viz property \prevTU
    function GetNextRT(): TBlkRT;                                               // vrati dalsi TU v zavislosti na smeru trati, pokud smer neni AtoB nebo BtoA, vrati nil, viz property \nextTU
    ...
    function GetSectMaster(): TBlkRT;                                           // vrati sectMaster kazdeho TU, pokud je TU sam sobe sectMaster, obsahuje referenci na self, viz property \sectMaster
    function GetNextSignal(): TBlk;                                             // vrati dalsi navestidlo v trati, v krajnim pripade az hranicni navestidlo cele trati podle aktualniho smeru trati, viz property \nextNav
  ...
  public
   lTU, sTU: TBlkRT;                                                            // reference na tratovy usek blize zacatku trati (lTU) a TU blize konci trati (sTU), tyto refence nastavuje trat pri inicializaci, nebo zmene konfigurace trati

   lsectMaster: TBlkRT;                                                         // sectMaster pro lichy smer trati
   lsectTracks: TList<TBlkRT>;                                                  // pokud jsem sectMaster, zde jsou ulozeny useky me sekce v lichem smeru; pokud nejsem sectMaster, je tento senzam prazdny

   ssectMaster: TBlkRT;                                                         // sectMaster pro sudy smer trati
   ssectTracks: TList<TBlkRT>;                                                  // pokud jsem sectMaster, zde jsou ulozeny useky me sekce v sudem smeru; pokud nejsem sectMaster, je tento senzam prazdny
...
begin
 ...
 if ((ini_tech.ReadString(section, 'zast_ev_lichy_zast', '') <> '') or
   ...
   if (Assigned(Self.m_tuSettings.stop)) then
     Self.m_tuSettings.stop.Free();
end;
...
begin
 ...
 if (Self.railway <> nil) then
  begin
   ...
   if ((Self.sectMaster = Self) and (Self.prevRT <> nil) and
       (Self.prevRT.sectMaster <> nil)) then Self.prevRT.sectMaster.Change();
  end;
end;
...
begin
 ...
 if (((Self.railway as TBlkRailway).direction >= TRailwayDirection.AtoB) and (Self.prevRT = nil) and (Self.nextRT = nil)) then
...
begin
 if (success) then
  begin
   ...
   if (Self.railway <> nil) then Self.railway.Change();
  end;
end;
...
begin
 if ((Self.railway = nil) or ((TBlkRailway(Self.railway).direction <> TRailwayDirection.AtoB) and (TBlkRailway(Self.railway).direction <> TRailwayDirection.BtoA))) then Exit(nil);

 case (TBlkRailway(Self.railway).direction) of
   TRailwayDirection.AtoB : navPrevID := Self.m_tuSettings.signalLid;
   TRailwayDirection.BtoA : navPrevID := Self.m_tuSettings.signalSid;
 else
  navPrevID := -1;
 end;
...
begin
 if (not Self.railwayReady) then Exit(nil);
...
begin
 if ((not Self.railwayReady) or (not TBlkRailway(Self.railway).BP)) then Exit();
 ...
 if ((Self.prevRT <> nil) and (Self.occupied = TTrackState.occupied) and
     (Self.prevRT.occupied = TTrackState.occupied) and
     ((Self.signalCover = nil) or (TBlkSignal(Self.signalCover).IsGoSignal()))) then
  begin
 ...
 if ((Self.bpInBlk) and (Self.nextRT <> nil) and (Self.nextRT.train = Self.train) and
  ...
  begin
   Self.bpInBlk := false;
   Self.RemoveTrains();
   if (not TBlkRailway(Self.railway).occupied) then TBlkRailway(Self.railway).BP := false;
  end;
...
begin
 if (Self.railway = nil) then Exit(TTrackState.none);
 ...
 case (TBlkRailway(Self.railway).direction) of
  TRailwayDirection.AtoB : sectUseky := Self.lsectTracks;
  TRailwayDirection.BtoA : sectUseky := Self.ssectTracks;
...
begin
 if (Self.railway = nil) then Exit(nil);
 case (TBlkRailway(Self.railway).direction) of
...
var blk: TBlkRT;
begin
 if (Self.railway = nil) or (TBlkRailway(Self.railway).direction = TRailwayDirection.no) then Exit(nil);
 ...
 if (blk <> nil) then
  ...
  case (TBlkRailway(Self.railway).direction) of
    TRailwayDirection.AtoB : Exit(TBlkRailway(Self.railway).signalB);
    TRailwayDirection.BtoA : Exit(TBlkRailway(Self.railway).signalA);
  end;
 end;
...
begin
 ...
 if (Self.railway = nil) then Exit();

 // NASTAVOVANI NAVESTI AUTOBLOKU:

 // nejprve zhasneme navestidla v nespravnem smeru
 if ((TBlkRailway(Self.railway).direction = TRailwayDirection.AtoB) or (TBlkRailway(Self.railway).direction = TRailwayDirection.no)) then
  begin
   if (Self.m_tuSettings.signalSid > -1) then
    begin
     Blocks.GetBlkByID(Self.m_tuSettings.signalSid, Blk);
 ...
 if ((TBlkRailway(Self.railway).direction = TRailwayDirection.BtoA) or (TBlkRailway(Self.railway).direction = TRailwayDirection.no)) then
  begin
   if (Self.m_tuSettings.signalLid > -1) then
    begin
     Blocks.GetBlkByID(Self.m_tuSettings.signalLid, Blk);
   ...
   if (not Self.sectReady) then
    begin
     ...
     case (TBlkRailway(Self.railway).signals) of
       TRailwaySignals.hradlo: TBlkSignal(Self.signalCover).signal := ncVolno;
       TRailwaySignals.autoblok: begin
         if ((Self.nextSignal = nil) or (not TBlkSignal(Self.nextSignal).IsGoSignal()) or (TBlkSignal(Self.nextSignal).IsOpakVystraha())) then
           TBlkSignal(Self.signalCover).signal := ncVystraha
         else if ((TBlkSignal(Self.nextSignal).FourtyKmph()) or (TBlkSignal(Self.nextSignal).signal = ncOpakOcek40)) then
...
begin
 if ((state) and (Self.m_tuState.trainSpeedUpdateIter = 0)) then Self.m_tuState.trainSpeedUpdateIter := 2;
 if ((not state) and (Self.m_tuState.trainSpeedUpdateIter > 0)) then Self.m_tuState.trainSpeedUpdateIter := 0;
end;
...
begin
 if ((Self.railway = nil) or (Self.occupied <= TTrackState.none)) then Exit(false);

 case (TBlkRailway(Self.railway).direction) of
  TRailwayDirection.AtoB : sectTracks := Self.lsectTracks;
  TRailwayDirection.BtoA : sectTracks := Self.ssectTracks;
  TRailwayDirection.no : begin
         if (Self.sTU = nil) then
 ...
 for blk in sectTracks do
   if ((blk.occupied <> TTrackState.free) or (blk.IsTrain())
    or (blk.bpError)) then Exit(false);
 Result := true;
end;
...
begin
 ...
 Self.trainTypeRe.Compile(TBlkSignalTrainEvent.ParseTrainTypes(ini_tech.ReadString(section, 'zast_soupravy', '')), false);
end;
