  ...
  try
    ...
    on E:Exception do
     begin
      ...
      Application.MessageBox(PChar('Nelze naËÌst knihovnu mtb.dll:'+#13#10+E.Message), 'Nelze naËÌst knihovnu', MB_OK OR MB_ICONWARNING);
      writelog('Nelze naËÌst knihovnu mtb.dll: '+E.Message, WR_ERROR);
      Exit();
     end;
  end;
 ...
 begin
  ...
  try
    ...
    on E:Exception do
     begin
      Screen.Cursor := crDefault;
      Application.MessageBox(PChar('Nelze naËÌst knihovnu simulator.dll:'+#13#10+E.Message), 'Nelze naËÌst knihovnu', MB_OK OR MB_ICONWARNING);
      writelog('Nelze naËÌst knihovnu simulator.dll: '+E.Message, WR_ERROR);
      Exit();
     end;
  end;
  ...
  if (GetFunctions.GetQueryClose) then
   begin                                                 //pokud je mozno ukoncit program
    ...
    if (CloseMessage) then
     begin
      if Application.Messagebox('Opravdu chcete ukonËit program?','hJOPserver', MB_YESNO OR MB_ICONQUESTION OR MB_DEFBUTTON2) = mrYES then
       begin
        CanClose := true;
 ...
 try
   ...
   on E:Exception do
    begin
     ...
     Application.MessageBox(PChar('Chyba p¯i uzavÌr·nÌ MTB - '+E.Message),'Chyba',MB_OK OR MB_ICONERROR);
    end;
 end;
end;
 ...
 try
  ...
  on E:Exception do
   begin
    ...
    Application.MessageBox(PChar('Chyba pri otevÌr·nÌ MTB'+#13#10+E.Message),'Chyba', MB_OK OR MB_ICONERROR);
   end;
 end;
end;//procedure
  ...
  begin
   ...
   if (str <> '') then
    begin
     ...
     Self.LogStatus('WARN: ChybÌ MTB moduly '+str);
...
begin
 ...
 if (not MTB.ready) then
  begin
   Application.MessageBox(PChar('SystÈm nelze spustit, MTB nenÌ p¯ipraveno k zapnutÌ systÈmu'+#13#10+'MoûnÈ p¯ÌËiny:'+#13#10+' - nenaËtena validnÌ knihovna'), 'Nelze spustit', MB_OK OR MB_ICONWARNING);
   Self.LogStatus('ERR: SystÈm nelze spustit, MTB nenÌ p¯ipraveno k zapnutÌ systÈmu');
   Exit();
  end;
