...
uses ComCtrls, SysUtils, Generics.Collections, Classes, RCSsc, RCSc;
...
type
  ...
  public
    ...
    constructor Create(LV: TListView; rcs: TRCS);
    destructor Destroy(); override;
  end;

var
  RCSTableData: array [0..RCSs._RCSS_MAX] of TRCSTableData;

implementation

uses RCSIFace, IfThenElse;

/// /////////////////////////////////////////////////////////////////////////////

constructor TRCSTableData.Create(LV: TListView; rcs: TRCS);
begin
  inherited Create();
  Self.LV := LV;
  Self.rcs := rcs;
...
begin
  ...
  if (not rcs.ready) then
    Exit();

  for var i := 0 to rcs.maxModuleAddr do
  begin
    if (rcs.ready) then
      if ((not rcs.IsModule(i)) and (not rcs.GetNeeded(i)) and (not load_all)) then
        continue;
...
begin
  ...
  try
    if (rcs.IsModule(addr)) then
    begin
      var type_: string := rcs.GetModuleType(addr);
  ...
  LI.SubItems[0] := ite(rcs.GetNeeded(addr), 'X', '');

  try
    if (rcs.IsModule(addr)) then
      LI.SubItems[1] := rcs.GetModuleName(addr)
    else
      LI.SubItems[1] := '-';
  except
    on E: Exception do
  ...
  try
    if (rcs.IsModule(addr)) then
      LI.SubItems[2] := rcs.GetModuleType(addr)
    else
      LI.SubItems[2] := '-';
  except
    on E: Exception do
  ...
  try
    ...
    if (not rcs.IsNonFailedModule(addr)) then
    begin
      if (rcs.IsModuleFailure(addr)) then
        LI.SubItems[5] := 'Fail'
      else
        LI.SubItems[5] := '';
      LI.SubItems[6] := '-';
    ...
    if (rcs.IsModuleError(addr)) then
      LI.SubItems[5] := 'Error'
    else if (rcs.IsModuleWarning(addr)) then
      LI.SubItems[5] := 'Warning'
    else
      LI.SubItems[5] := '✓';

    LI.SubItems[6] := rcs.GetModuleFW(addr);
...
begin
  ...
  try
    if ((not rcs.ready) or (not rcs.Opened) or (not rcs.IsNonFailedModule(addr)) or (not rcs.Started)) then
    begin
      LI.SubItems[3] := '-';
      Exit();
    end;
    ...
    var cnt := rcs.GetModuleInputsCount(addr);
    if (cnt = 0) then
...
begin
  ...
  try
    ...
    var cnt := rcs.GetModuleOutputsCount(addr);
    if (cnt = 0) then
      Exit(); // Cannot decrement cnt-1, because cnt is Cardinal
    ...
    begin
      ...
      var output := rcs.GetOutput(addr, j);
      if (output > 1) then
