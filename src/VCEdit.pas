...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, Spin, Main, TechnologieJC, RPConst,
...
uses GetSystems, FileSystem, TBloky, TBlok, TOblsRizeni,
      TBlokSCom, Settings, TJCDatabase, DataJC, TBlokTrat;
 ...
 begin
  ...
  if (OpenIndex < 0) then
   begin
    ...
    if (return <> 0) then
     begin
      Application.MessageBox(PCHar('P¯id·v·nÌ JC skonËilo s chybou '+IntToStr(return)), 'Chyba', MB_OK OR MB_ICONWARNING);
      Exit;
     end;
